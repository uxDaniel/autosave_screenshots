...
class ScreenshotsMover:
    def __init__(
        ...
        preview_dimensions=[(728, 360), (455, 225)], # previews generated by Snipping Tool, to be ignored
        delay_seconds=2,
        do_cleanup=True,
    ):
    ...
    def move_screenshots(self, old_filenames, new_filenames):
        for i in range(len(old_filenames)):
            ...
            print(f"{source}\\{old_filenames[i]}  ➔   {target}\\{new_filenames[i]}")

    def main(self):
        imghdr.tests.append(ScreenshotsMover.test_jpeg)

        print(f"Monitoring {self.sourcepath}")
        while True:
